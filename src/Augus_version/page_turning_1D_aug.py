...
####################################################
def main_logic(x_value,z_value,theta_value):
  print "==========start1"
  
  for num_test in range(0,2):
...
#      move_target(qr_x,qr_y ,0.2,curr_x_ori,curr_y_ori,curr_z_ori,curr_w_ori,0.5)
      move_waypoints(qr_x, -qr_y, -0.2, 0.1)
      print "------->   apriltag distance ",qr_x," ",qr_y," <-----------"
      ...
      rospy.sleep(0.5)
      move_waypoints(-x_value,0.13,-0.1,0.4)
      ...
      curr_z_ori = group.get_current_pose().pose.orientation.z
      curr_w_ori = group.get_current_pose().pose.orientation.w
      move_target(curr_x,curr_y,0.125,curr_x_ori,curr_y_ori,curr_z_ori,curr_w_ori,0.4)
      rospy.sleep(0.5)
...
def start_robot():
  ...
  for k in range(0,1):
    for m in range(0,7):
      for n in range(0,7):
        print "test"
        main_logic(0.03+m*0.01,0.11+n*0.005,k+0.05)
