...
#####################################################################
def move_third_joint(degree):
       group_variable_values=group.get_current_joint_values()
       group_variable_values[3]-=degree*pi/180
       group.set_joint_value_target(group_variable_values)
       plan=group.plan()
       rospy.sleep(0.08)
       trajectory=scale_trajectory_speed(plan,0.2)
       group.execute(plan)
    ...
    if cam_pose.detections[0].id == 3:
      ...
      if abs(qr_x)>0.002 or abs(qr_y)>0.002:
        ...
        if abs(qr_x)>0.002 or abs(qr_y)>0.002:
          ...
          print('das ist !')
      rospy.sleep(0.5)
      move_waypoints(-x_value,0.05,-0.1,0.2)
      rospy.sleep(1)
...
def start_robot():
  ...
  for k in range(9,10):  # theta value
    for m in range(5,6): # x value
      for n in range(7,8): # z value
        main_logic(0.0+m*0.01-0.004,0.100+n*0.001,k+0.001,1)
