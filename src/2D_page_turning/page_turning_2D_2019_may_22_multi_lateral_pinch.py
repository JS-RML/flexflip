...
def page_turn():
  ...
  arduino_pub = rospy.Publisher('/soft', Int16, queue_size=1)
  rospy.sleep(0.2)
  arduino_pub.publish(0037)
  rospy.sleep(3)

  arduino_pub = rospy.Publisher('/soft', Int16, queue_size=1)
  rospy.sleep(0.2)
  arduino_pub.publish(5037)
  rospy.sleep(5)

  move_waypoints(0,0,0.01,0.1)
  arduino_pub = rospy.Publisher('/soft', Int16, queue_size=1)
  rospy.sleep(0.2)
  arduino_pub.publish(4242)
  rospy.sleep(3)

  move_waypoints(0,0.04,0.02,0.1)
  rospy.sleep(2)

  move_frame(0, 0, 0,0,-30*pi/180,0,0.2, '/soft_gripper')
  rospy.sleep(0.5)
  move_frame(0, 0, 0,75*pi/180,0,0,0.2, '/soft_gripper')
  rospy.sleep(0.5)

  arduino_pub.publish(4848)
  rospy.sleep(3)

  move_waypoints(0,0.06,0.06,0.1)
  rospy.sleep(0.5)

  move_waypoints(0,0.40,0,0.1)
  rospy.sleep(1)
  arduino_pub = rospy.Publisher('/soft', Int16, queue_size=1)
  rospy.sleep(2)
...
def main_logic(x_value,z_value,theta_value,i_test):
  ...
  for num_test in range(0,i_test):
    ...
    if cam_pose.detections[0].id == 3:
      ...
      rospy.sleep(0.5)
      move_waypoints(-0.05,-0.09,0.0,0.4)
      rospy.sleep(0.5)

      angle=theta_value*pi/180
      print"angle", angle

      move_frame(0, 0, 0,15*pi/180,0,0,0.2, '/soft_gripper')
      rospy.sleep(2)
      move_frame(0, 0, 0,0,40*pi/180,0,0.2, '/soft_gripper')
      rospy.sleep(0.5)

      move_frame(0, 0, 0,0,0,angle,0.2, '/soft_gripper')
      rospy.sleep(0.5)
      move_waypoints((0.05-x_value),(-0.04+x_value),0,0.4)
      rospy.sleep(0.5)
      ...
      move_target(curr_x,curr_y ,z_value,curr_x_ori,curr_y_ori,curr_z_ori,curr_w_ori,0.1)
  ...
  for k in range(0,1):  # theta value
    for m in range(4,5): # x value
      for n in range(0,1): # z value
        main_logic(0.005+m*0.01,0.131+n*0.001,k+0.001,5)
