...
def main_logic(x_value,z_value,theta_value,i_test):
  ...
  for num_test in range(0,i_test):
    ...
    if cam_pose.detections[0].id == 3:
      ...
      rospy.sleep(0.5)
      move_waypoints(-x_value,0.05,-0.1,0.4)
      rospy.sleep(1)
      ...
      move_target(curr_x,curr_y ,z_value,curr_x_ori,curr_y_ori,curr_z_ori,curr_w_ori,0.2)
  ...
  for k in range(0,1):  # theta value
    for m in range(3,4): # x value
      for n in range(0,1): # z value
        main_logic(0.001+m*0.01,0.113+n*0.001,k+0.001,5)
