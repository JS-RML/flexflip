...
def page_turn():
  ...
  arduino_pub = rospy.Publisher('/soft', Int16, queue_size=1)
  rospy.sleep(3)
  arduino_pub.publish(5030)
  rospy.sleep(3)

  rospy.sleep(0.5)
  move_waypoints(0,0,0.02,0.1)
  arduino_pub = rospy.Publisher('/soft', Int16, queue_size=1)
  rospy.sleep(3)
  arduino_pub.publish(5050)
  rospy.sleep(1) 
  
  move_waypoints(0,0,0.15,0.1)
...
def main_logic(x_value,z_value,theta_value,i_test):
  ...
  for num_test in range(0,i_test):
    ...
    if cam_pose.detections[0].id == 3:
      ...
      print"angle", angle 
      move_frame(0, 0, 0,0,45*pi/180,0,0.2, '/soft_gripper')
      rospy.sleep(0.5)
      move_frame(0, 0, 0,0,0,angle,0.2, '/soft_gripper')
      rospy.sleep(0.5)
      move_waypoints((0.04-x_value),-(0.07-x_value),0,0.4)
      rospy.sleep(0.5)
  ...
  for k in range(0,1):  # theta value
    for m in range(4,5): # x value
      for n in range(0,1): # z value
        main_logic(0.005+m*0.01,0.112+n*0.001,k+0.001,5)
